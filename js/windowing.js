...
function openSecureInnerWindow (name, url, width, height, title, params, x, y, titlebar) {
  var window = new SecureInnerWindow(name, url, width, height, title, params, x, y, titlebar);
  window.open();
}

function openClientWindow (name, contents, width, height, title, x, y, titlebar, screen) {
  var window = new ClientInnerWindow(name, contents, width, height, title, x, y, titlebar);
  if (screen)
    openScreen(name);
  window.open();
...
InnerWindow = function (name, url, width, height, title, params, x, y, titlebar) {
  this.name = name;
  this.url = url;
...
InnerWindow.prototype = {
  ...
  getTitlebar:function () {
    ...
    titlebar.innerHTML = "<A href=\"javascript: closeInnerWindow('" + this.name + "')\" style=\"float: right;\">close [ x ]</A>&nbsp;<!-- <IMG src=\"images/minimize.gif\" id=\"" + this.name + "_minimize_button\" onClick=\"windowManager.minimizeWindow('" + this.name + "')\"> -->&nbsp;<B>" + this.title + "</B></DIV>";
...
SecureInnerWindow.prototype.loadContents = function () {
  if (this.titlebar) {
    ...
    setStyle(secureIndicator, "margin-top: 3px; background: url('images/caution.jpg');");
    var lock = document.createElement("IMG");
    lock.setAttribute("id", this.name + "_lock_icon");
    lock.setAttribute("src", "images/lock_small.jpg");
  ...
  setStyle(contentFrame, "width: " + this.width + "px; border: #000000 solid 0px; height: " + this.height + "px;");
...
ClientInnerWindow = function (name, content, width, height, title, x, y, titlebar) {
  InnerWindow.call(this);
  this.name = name;
