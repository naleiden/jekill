...
function loadAssociatedRecords (tableName, fieldName, entityID) {
	$.post("schema/load_associated_rows.php", { table_name: tableName, field_name: fieldName, entity_ID: entityID }, function (response) {
...
function annotateImage (parentTable, imageField, parentID, annotationTable, annotationSuffix) {
	...
	openInnerWindow("annotate_image", "schema/annotate_image.php", 850, 675, "Annotate Image", params);
...
function browseFiles (parentField, directory, extensions, callback) {
	...
	openInnerWindow("browse_files", "kernel/browse_files.php", 500, 400, "File Selector", "directory=" + directory + "&extensions=" + extensions + "&callback=" + callback);
}

function refreshImagesPreview (parentField) {
	var serialized = $("#" + parentField).val();

	$.post("schema/get_images_preview_divide.php", { field_name: parentField, serialized: serialized }, function (response) {
...
function formatImage (imageURL, width, height, outputURL, callback) {
	...
	openInnerWindow("format_image", "schema/embedded_image_manipulator.php", 970, 600, "Crop Image", params);
}
...
function addSubtableRecord (tableName, fieldName, recordID, suffix) {
	...
	$.post("schema/add_subtable_record.php", arguments, function (response) {
...
function undeleteSubtableRecord (fieldName, tableName, entityID, recordNum) {
...
function deleteSubtableRecord (fieldName, tableName, suffix, entityID, recordNum) {
	...
	$("#" + tableName + "_" + fieldName + suffix + "_" + recordNum).css("background-color", "#CC9999");	// .addClass("deleted");

/*  OLD WAY OF DELETING
	else {
		decrementNumSubtableRecords(fieldName);
		$.post("schema/delete_entity.php", { table: tableName, entity_ID: entityID }, function () { deleteSubtableRecordCallback(fieldName, tableName, recordNum, "#CC0000"); });
	}
*/
}
...
function searchExistingSubtableRecords (fieldName, tableName) {
	...
	$.post("schema/get_subtable_records.php", { field_name: fieldName, table_name: tableName, search: search }, function (response) { $("#subrecord_chooser").html(response); });
}
...
/* OLD WAY OF DISASSOCIATING
	$.post("schema/unlink_subrecord.php", { table_name: tableName, field_name: fieldName, entity_ID: recordID, link_ID: subrecordID }, function () { deleteSubtableRecordCallback(tableName, fieldName, recordNum, "#CCCC00"); });
*/
}
