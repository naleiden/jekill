...
function login (jekillRoot, successFunc, passwordFunction) {
	var userType = $("#user_type").val();
	var loginType = $("#login_type").val();
	var authentication = $("#" + userType + "_authentication").val();
	var userID = $("#login_" + loginType).val();
	var password = $("#login_password").val();
	var remainLoggedIn = $("#remain_logged_in").attr("checked") ? true : false;

	var formattedPassword = password;
	if (typeof(passwordFunction) == "function")
		formattedPassword = passwordFunction(password);
	...
	$.post(jekillRoot + "/users/login_script.php", { user_authentication: authentication, login_type: loginType, user_type: userType, user_identifier: userID, password: formattedPassword, remain_logged_in: remainLoggedIn }, function (response) { loginCallback(response, successFunc); });
}
